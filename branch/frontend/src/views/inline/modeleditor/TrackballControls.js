...
var TrackballControls = function (object, domElement) {
  ...
  var _state = STATE.NONE,
    ...
    _translateStart = new Vector2(),
    _translateEnd = new Vector2();
  ...
  this.zoomCamera = function () {
    ...
    if (_state === STATE.TOUCH_ZOOM_TRANSLATE) {
  ...
  this.translateCamera = (function () {
    ...
    return function translateCamera() {
      mouseChange.copy(_translateEnd).sub(_translateStart);
      ...
      if (mouseChange.lengthSq()) {
        ...
        mouseChange.multiplyScalar(_eye.length() * _this.translateSpeed);

        translate.copy(_eye).cross(_this.object.up).setLength(mouseChange.x);
        translate.add(objectUp.copy(_this.object.up).setLength(mouseChange.y));

        _this.object.position.add(translate);
        _this.target.add(translate);

        if (_this.staticMoving) {
          _translateStart.copy(_translateEnd);
        } else {
          _translateStart.add(
            mouseChange
  ...
  this.checkDistances = function () {
    if (!_this.noZoom || !_this.noTranslate) {
  ...
  function keydown(event) {
    ...
    if (_keyState !== STATE.NONE) {
      ...
      event.keyCode === _this.keys[STATE.TRANSLATE] &&
      !_this.noTranslate
    ) {
      _keyState = STATE.TRANSLATE;
    }
  }
  ...
  function mousedown(event) {
    ...
    } else if (state === STATE.TRANSLATE && !_this.noTranslate) {
  ...
  function touchstart(event) {
    ...
    switch (event.touches.length) {
      ...
      default:
        // 2 or more
        _state = STATE.TOUCH_ZOOM_TRANSLATE;
        ...
        _translateStart.copy(getMouseOnScreen(x, y));
        _translateEnd.copy(_translateStart);
        break;
    }
  ...
  function touchmove(event) {
    ...
    switch (event.touches.length) {
      ...
      default:
        ...
        var x = (event.touches[0].pageX + event.touches[1].pageX) / 2;
        var y = (event.touches[0].pageY + event.touches[1].pageY) / 2;
        _translateEnd.copy(getMouseOnScreen(x, y));
        break;
    }
  }
